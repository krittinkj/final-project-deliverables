...
During system testing, we encountered a problem with the official UR ROS2 driver used to control the UR3e arm. After investigating the issue, we found that it corresponds to an open bug in the official driver that affects motion execution reliability~\cite{ur-driver-issue}. Since the issue remains unresolved and its timeline is uncertain, relying solely on the driver introduces a potential risk to system stability.

To mitigate this risk, we implemented a backup control mechanism by developing a new implementation of the \texttt{low\_level\_planner\_executor} node that communicates with the robot using \textit{URScript}. URScript is the native scripting language used by Universal Robots controllers, designed to directly command robot motion, gripper actions, and I/O operations through programmatic instructions executed by the robot controlle~\cite{ur-script}.

Instead of relying on the ROS2 driver abstraction, this node sends URScript commands directly to the robot's \textit{URControl} interface using TCP/IP socket communication. This approach follows the standard external control mechanism supported by Universal Robots and allows motion commands to be executed without the ROS2 driver middleware. The method is also consistent with techniques introduced in earlier coursework, where URScript programs are transmitted over a socket connection to the robot controller.
