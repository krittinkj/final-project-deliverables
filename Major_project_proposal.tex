...
The current progress of the perception module focuses on stabilizing the object detection pipeline integrated with the Intel RealSense D435 depth camera in order to convert 2D image pixels into accurate 3D real-world coordinates using ROS~2 and the librealsense SDK. The development process involved three major phases: environment stabilization, perception debugging, and implementation of the pixel-to-real transformation service.

During the initial setup, a Python dependency conflict was encountered due to a version mismatch of \texttt{setuptools}. The virtual environment (\texttt{vision\_venv}) required version 59.6.0, whereas the global user-site installation contained version 80.10.2. This discrepancy caused package uninstall errors and environment instability because Python referenced user-site packages even inside the virtual environment. The issue was resolved by isolating the environment using \texttt{PYTHONNOUSERSITE=1} and removing conflicting global packages. After this adjustment, dependency resolution and package management became stable.

Subsequently, issues were observed in the visualization pipeline where the GraspNet and Scene Understanding windows appeared black and unresponsive, despite successful ROS topic publishing from \texttt{/camera/color/image\_raw} and verified QoS compatibility. Depth and RGB streams were confirmed functional, and CvBridge conversion was inspected. The root cause was identified as a version mismatch between the system-installed \texttt{cv\_bridge} package and the locally used OpenCV version. Since \texttt{cv\_bridge} was compiled against a different OpenCV build, runtime incompatibility occurred. To ensure consistency, the \texttt{vision\_opencv} repository was cloned and compiled from source against the specific OpenCV version used in the project. The CMake configuration was explicitly linked with the correct OpenCV library. After rebuilding the workspace, image visualization was restored and the perception pipeline became stable.

An additional issue occurred within the CLIP classifier node, producing the error \texttt{AttributeError: 'BaseModelOutputWithPooling' object has no attribute 'norm'}. This was traced to incompatibility between the HuggingFace CLIP implementation and the expected OpenAI CLIP API behavior in the pipeline. The solution was to replace the HuggingFace implementation with the original OpenAI CLIP repository installed directly from source. After this modification, object classification functionality was restored without further runtime errors.

For the pixel-to-real world coordinate conversion, the initial implementation relied on a ratio-based scaling approach derived from simulation measurements. This method estimated real-world distances by referencing known dimensions within the simulated environment and computing a pixel-to-world scaling factor. However, this approach was environment-dependent, lacked generalizability, and produced limited accuracy when transitioning to real hardware.

A manual camera calibration procedure was attempted using a $6\times7$ chessboard pattern. The resulting distortion coefficients were $k_1 = 0.0187$, $k_2 = 0.8990$, and $k_3 = -3.9033$. The large magnitude of $k_3$ indicated unreliable calibration, primarily due to incomplete field-of-view coverage during image acquisition. The distorted checkerboard did not sufficiently span the full image region, resulting in unstable parameter estimation. Consequently, manual calibration was deemed unsuitable for this setup.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\linewidth]{chess_rs_calibration.jpg}
    \caption{6x7 Chessboard calibration setup for Intel RealSense D435i, causing unreliable parameter estimation due to incomplete field-of-view coverage.}
    \label{fig:calibration_chess_setup}
\end{figure}

Given that the Intel RealSense D435 is factory calibrated and provides intrinsic parameters through the ROS \texttt{/camera\_info} topic, the methodology was revised to utilize the built-in camera intrinsics and the librealsense deprojection function. Instead of manually computing geometric transformations, the implementation now uses \texttt{rs2\_deproject\_pixel\_to\_point()}, which converts a 2D pixel coordinate and its associated depth value into a 3D point in the camera coordinate frame. The required inputs include the intrinsic parameters (focal lengths $f_x$, $f_y$, principal point $p_x$, $p_y$, and distortion coefficients), the pixel coordinate $(u,v)$, and the depth value at that location extracted from the aligned depth image.

Since the RealSense coordinate system differs from the ROS coordinate convention, a coordinate transformation was implemented to ensure compatibility with RViz visualization and downstream robotic manipulation tasks. The mapping follows: $X_{ROS} = Z_{RS}$, $Y_{ROS} = -X_{RS}$, and $Z_{ROS} = -Y_{RS}$. This transformation aligns the forward, left, and upward directions according to ROS standards.

To evaluate accuracy, a benchmarking experiment was initiated using a green box placed within the camera workspace as a physical reference object. The real-world distance was measured manually and compared against the output from the pixel-to-real-world service. The experimental setup is illustrated in Figure~\ref{fig:calibration_vision_setup}. Multiple trials are being conducted to compute absolute error and mean absolute error for quantitative validation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\linewidth]{calibration_vision_setup.jpg}
    \caption{Calibration setup showing the green and pink box used for validating pixel-to-real-world coordinate accuracy.}
    \label{fig:calibration_vision_setup}
\end{figure}

Following the stabilization of the intrinsic-based deprojection pipeline, further testing was conducted using the native \texttt{rs2\_deproject\_pixel\_to\_point()} function provided by the RealSense SDK. This implementation required subscribing to additional topics and restructuring the perception node to directly interface with the librealsense library. Although the deprojection logic was successfully implemented, repeated camera initialization issues were encountered during runtime. The failures were primarily related to device access timing conflicts and pipeline re-initialization when multiple nodes attempted to access the camera stream simultaneously. 

Due to project timeline constraints and the need to prioritize integration with the robot motion planning module, a decision was made to adopt a more stable and controllable approach based on explicit geometric computation using the pinhole camera model. Instead of relying on SDK-level deprojection, 3D coordinates were computed analytically from intrinsic parameters and depth values using standard perspective projection equations.

where $(u,v)$ represents the pixel coordinate, $Z$ is the measured depth value, and $(f_x, f_y, p_x, p_y)$ are intrinsic parameters obtained from the camera calibration.

Experimental validation was performed using ten objects placed at ten different locations within the working space. All measurements were conducted using the Intel RealSense D435i depth camera after calibration alignment. The computed real-world coordinates were compared against manually measured ground-truth values. The evaluation resulted in a Root Mean Square Error (RMSE) of $0.0494$~m. While this result demonstrates functional consistency of the geometric transformation approach, further refinement is required to improve accuracy and reduce residual error within the workspace.

Furthermore, the \texttt{pixel\_to\_real\_world} service was integrated into the \texttt{find\_service} module. This integration enables the system to detect an object in the RGB image, extract its pixel location, compute its 3D coordinate using geometric transformation, and return the position relative to the Universal Robots (UR) arm base frame. The UR arm origin is defined as $(0,0,0)$, while the calibrated depth camera position relative to the robot base is $(0, -0.5442, 0.6711)$. The final world coordinate is therefore computed by combining camera-frame coordinates with the fixed extrinsic transformation between the camera and robot base.

Although the geometric model provided a stable solution for integration with the robot planning module, further improvements are planned. In particular, additional investigation into the RealSense SDK-based deprojection method will be conducted when time permits, with the objective of resolving the previous camera initialization conflicts and potentially improving overall spatial accuracy.

The coordinate transformation process is illustrated in Figure~\ref{fig:pixel_to_world_pipeline}, showing the image-space detection alongside its corresponding 3D 
(x,y,z) coordinate output in the robot reference frame.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{find_object_real_world.jpg}
    \caption{Image for converting detected bounding box pixel into UR arm base-frame coordinates using geometric transformation and calibrated camera extrinsics.}
    \label{fig:pixel_to_world_pipeline}
\end{figure}

This approach provided greater stability during integration with real world testing and ensured deterministic behavior without camera reinitialization conflicts. Ongoing improvements focus on refining extrinsic calibration and reducing cumulative transformation error.
...
Since the main focus of the early part of the second semester is integrating and moving the setup from Gazebo simulation to actual hardware, the planning component has not undergone major architectural changes. However, we have conducted extensive research on PDDL generation approaches to understand the current state of the field and identify techniques that could improve our system's reliability and correctness. We surveyed recent work and found that most approaches share a common foundation: using LLMs for PDDL generation with different techniques such as prompt engineering, feedback loops, multi-shot prompting, and few-shot prompting. Even ontology-based approaches like PlanOwl~\cite{planowl} leverage ontologies for environmental knowledge representation but still rely on an LLM for the final PDDL generation step. We identified five principal approaches: LLM-based structured generation (ViLaIn~\cite{vilain}, Image2PDDL~\cite{image2pddl}, LLM+P~\cite{llm+p}), fine-tuned LLMs (PDDL-Instruct~\cite{pddl-instruct}), vision-to-symbolic pipelines (VeriGraph~\cite{verigraph}, SayPlan~\cite{sayplan}), neuro-symbolic domain learning (PDDLLM~\cite{pddllm}, UniDomain~\cite{unidomain}), and planner-in-the-loop multi-agent frameworks (SPCA~\cite{spca}, LLM Domain Generator~\cite{llm-domain-generators}). Our analysis reveals that while prompted LLM approaches are flexible, they suffer from hallucinated predicates, missing preconditions, and long-horizon consistency failures. Hybrid neuro-symbolic systems with planner-in-the-loop validation~\cite{pddllm,unidomain, spca} consistently outperform prompted-only methods, with critical correctness enforcement techniques including syntax validation through prompt templates, semantic validation via planner solvability checks, action validity through simulation rollouts, and logical reasoning verification.

Informed by this research, we have improved our PDDL generation through enhanced prompt engineering and flexible template-based approaches. Previously, we used a static PDDL structure for robot actions and predicates. However, testing revealed that this approach lacked flexibility, and the LLM often struggled to model sequential tasks requiring modifications to action preconditions and effects. We now provide a manually created PDDL template in the system prompt that the LLM can reference and modify as needed, similar to the structured prompting strategies employed in Image2PDDL~[2] and ViLaIn~[1]. This change has improved performance in preliminary tests, though extensive evaluation is still required. We are also exploring planner-in-the-loop validation inspired by ViLaIn~[1] and SPCA~[9] to catch semantic errors early.

During testing, we encountered a practical constraint: for complex, multi-step instructions, we are hitting the input token limit of our LLM API's free tier. We plan to address this by either making our action primitives more complex to reduce function calls, making the medium-level planner more deterministic to limit API usage, and optimizing the system prompt. 

\subsubsection{Hardware}

At the current stage of development, the hardware infrastructure for the vision-guided manipulation system has been partially implemented and validated within a controlled laboratory environment. The system is built around a Universal Robots (UR) collaborative robotic arm mounted on a fixed experimental table.

To enable perception-driven manipulation, a depth camera holder was mechanically installed at the base of the UR arm. The mounting structure was secured to ensure positional stability and minimize vibration during operation. An Intel RealSense D435i depth camera was attached to the holder and connected to the processing unit via a USB interface.

To accommodate the physical distance between the camera mounting location and the processing workstation, an extended USB cable was procured. The cable was routed carefully to avoid interference with the robot's operational workspace and to ensure safe operation during manipulation tasks.

Since the Intel RealSense D435i requires USB~3.0 bandwidth for stable RGB and depth data streaming, a USB~3.0-compatible hub/dongle was integrated to guarantee interface compatibility with the host system. This configuration ensured sufficient data transfer rates for real-time image acquisition without frame drops or communication instability. The connectivity setup was validated during continuous operation tests.

A blue board was placed within the robot's workspace to serve as a controlled visual background. This improves contrast for the perception node and reduces noise in segmentation and depth estimation tasks. The physical installation of the camera mount and workspace preparation is shown in Figure~\ref{fig:camera_mount_setup}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{camera_mount_setup.jpg}
    \caption{Experimental hardware setup showing the installed depth camera mount at the UR arm base and the blue board placed within the manipulation workspace.}
    \label{fig:camera_mount_setup}
\end{figure}

\paragraph{Coordinate System Definition}

To enable consistent spatial reasoning between perception and manipulation modules, a unified coordinate reference frame was defined. All coordinates are expressed relative to the origin located at the center of the UR arm base.

The reference points are defined as follows:

\begin{itemize}
    \item \textbf{UR Arm Base (Origin):} $(0,0,0)$
    \item \textbf{Depth Camera Position:} $(0,-0.5442,0.6711)$
\end{itemize}

The depth camera position specifies its spatial location relative to the robot base frame and was determined through direct physical measurement.

The spatial configuration and key geometric measurements are illustrated in Figure~\ref{fig:hardware_measurement}. This explicit coordinate definition establishes the foundation for camera-to-robot transformation and pose estimation during manipulation tasks.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{measurement_setup.jpg}
    \caption{Hardware measurement setup showing the UR arm base reference frame, depth camera position, table height, and spatial offsets. All coordinates are defined relative to the UR arm base origin $(0,0,0)$.}
    \label{fig:hardware_measurement}
\end{figure}

\paragraph{Key Physical Measurements}

Several physical parameters were measured to validate spatial consistency within the perception-to-action pipeline. These measurements were required to ensure accurate transformation between camera coordinates and the robot base frame.

The primary measured values include:

\begin{itemize}
    \item Vertical distance from UR base to camera mount
    \item Horizontal offset between camera and robot base center
    \item Table height relative to robot base
\end{itemize}

These parameters were incorporated into the system calibration process and are reflected in the coordinate definitions provided above.
...
\subsubsection{Planning Algorithm}
The current progress is on schedule. Since the primary focus has been on transitioning from simulation to hardware, the planning architecture remains stable while improvements have been made to PDDL generation through enhanced prompt engineering and flexible template-based approaches. The system can now generate more robust task plans for sequential operations, though ongoing work aims to address API usage constraints and improve scalability. Development continues toward seamless integration of high-level planning with the perception and execution modules within the hierarchical system architecture.
