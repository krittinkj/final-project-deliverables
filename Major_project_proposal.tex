...
\end{figure}
Given that the Intel RealSense D435 is factory calibrated and provides intrinsic parameters through the ROS \texttt{/camera\_info} topic, the initial approach was to leverage the built-in camera intrinsics together with the librealsense deprojection function. Specifically, the implementation experimented with \texttt{rs2\_deproject\_pixel\_to\_point()}, which converts a 2D pixel coordinate and its associated depth value into a 3D point in the camera coordinate frame. The required inputs include the intrinsic parameters (focal lengths $f_x$, $f_y$, principal point $p_x$, $p_y$, and distortion coefficients), the pixel coordinate $(u,v)$, and the depth value extracted from the aligned depth image.

Since the RealSense coordinate system differs from the ROS coordinate convention, a coordinate transformation was implemented to ensure compatibility with RViz visualization and downstream robotic manipulation tasks. The mapping follows:
\[
X_{ROS} = Z_{RS}, \quad
Y_{ROS} = -X_{RS}, \quad
Z_{ROS} = -Y_{RS}
\]
aligning the forward, left, and upward directions according to ROS standards.

To evaluate accuracy, a benchmarking experiment was conducted using a green box placed within the camera workspace as a physical reference object. The real-world distance was measured manually and compared against the output from the pixel-to-real-world service. Multiple trials were performed to compute absolute error and mean absolute error for quantitative validation. The experimental setup is illustrated in Figure~\ref{fig:calibration_vision_setup}.
...
Although the deprojection logic using \texttt{rs2\_deproject\_pixel\_to\_point()} was successfully implemented, repeated camera initialization issues were encountered during runtime. These failures were primarily caused by device access conflicts and pipeline re-initialization when multiple ROS nodes attempted to access the camera stream simultaneously. As a result, the SDK-based approach was deemed unstable for the current system configuration.

Due to project timeline constraints and the need to prioritize integration with the robot motion planning module, the implementation was temporarily shifted to a more stable and controllable method based on explicit geometric computation using the pinhole camera model. Instead of relying on SDK-level deprojection, 3D coordinates were computed analytically from intrinsic parameters and depth values using standard perspective projection equations:
\[
X = \frac{(u - p_x)Z}{f_x}, \quad
Y = \frac{(v - p_y)Z}{f_y}, \quad
Z = Z
\]
where $(u,v)$ represents the pixel coordinate, $Z$ is the measured depth value, and $(f_x, f_y, p_x, p_y)$ are intrinsic parameters obtained from the camera calibration. The SDK-based deprojection approach will be revisited in future work once system-level camera access and synchronization issues are resolved.
...
Informed by this research, we have improved our PDDL generation through enhanced prompt engineering and flexible template-based approaches. Previously, we used a static PDDL structure for robot actions and predicates. However, testing revealed that this approach lacked flexibility, and the LLM often struggled to model sequential tasks requiring modifications to action preconditions and effects. We now provide a manually created PDDL template in the system prompt that the LLM can reference and modify as needed, similar to the structured prompting strategies employed in Image2PDDL~\cite{image2pddl} and ViLaIn~\cite{vilain}. This change has improved performance in preliminary tests, though extensive evaluation is still required. We are also exploring planner-in-the-loop validation inspired by ViLaIn~\cite{vilain} and SPCA~\cite{spca} to catch semantic errors early.
