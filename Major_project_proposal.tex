...
\begin{itemize}
    ...
    \item \textbf{Ground Origin:} $(0,0,-0.805)$
    \item \textbf{Depth Camera Position:} $(0,-0.5442,0.6711)$
\end{itemize}

The ground origin represents the point directly beneath the center of the UR arm base, measured from the surface of the experimental table. The depth camera position specifies its spatial location relative to the robot base frame.
...
Several physical parameters were measured to validate spatial consistency within the perception-to-action pipeline.

\begin{table}[h]
\centering
\caption{Key Physical Measurements}
\begin{tabular}{|l|c|p{5cm}|}
\hline
\textbf{Parameter} & \textbf{Value} & \textbf{Description} \\ \hline
Future Board Width & 0.29 m & Width of the experimental board component \\ \hline
OFFSET & 0.1778 m & Distance from the center of the UR arm base to the center of the camera mounting point \\ \hline
HEIGHT & 0.805 m & Distance from the ground to the surface of the experimental table \\ \hline
\end{tabular}
\end{table}

These measurements were obtained using manual measurement tools and are used to verify the consistency of coordinate transformations implemented within the system.
...
\begin{itemize}
    \item The depth camera is securely mounted and operational.
    \item The camera position relative to the UR base has been measured.
    \item A consistent coordinate reference frame has been established.
    \item The experimental workspace has been prepared for controlled perception experiments.
\end{itemize}

This hardware configuration establishes the foundational infrastructure required for implementing accurate camera-to-robot coordinate transformation and enabling vision-guided manipulation tasks in subsequent development phases.
\newpage
