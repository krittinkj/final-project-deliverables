...
The vision pipeline has been implemented as a modular, end-to-end perception system and fully integrated into a ROS2 service-oriented architecture. It consists of four sequential stages: object segmentation using the Segment Anything Model (SAM), zero-shot object classification using CLIP, 6-DoF grasp synthesis using GraspNet, and scene understanding for spatial reasoning. Segmentation outputs are refined through a mask-merging strategy to reduce over-segmentation, enabling more stable and confident classification. Grasp candidates are generated for segmented objects using synthetic RGB-D inputs, and a scene understanding module infers relative spatial relationships between objects. Each capability is deployed as an independent ROS2 node with on-demand services, allowing the complete pipeline to be triggered via a single service call and seamlessly interfaced with downstream motion planning and control modules.
