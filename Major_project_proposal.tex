...
During testing, we encountered a practical constraint: for complex, multi-step instructions, we are hitting the request limit of our LLM API's free tier. We plan to address this by either making our action primitives more complex to reduce function calls, or making the medium-level planner more deterministic to limit API usage. Overall, the planning module's progress remains on schedule according to the initial timeline, with ongoing refinements focused on correctness, scalability, and robustness.
