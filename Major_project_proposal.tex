...
\textbf{ROS2 Service Node Integration} \\

With the core perception modules completed, the next phase of development focused on transforming the entire system into a fully modular ROS2 service-oriented architecture. All functionality previously written in standalone Python scripts was migrated into ROS2 nodes using rclpy, with image conversion handled through \texttt{cv\_bridge}. This redesign ensures that each step of the pipeline is callable on demand and compatible with real-time robotic control.

\textbf{Overview of Nodes} 

\begin{tabular}{|l|l|p{7cm}|}
\hline
\textbf{Node} & \textbf{Role} & \textbf{Key Topics / Services} \\
\hline
simple\_sam\_detector & Detects objects & \texttt{/vision/run\_pipeline}, \texttt{/vision/detect\_objects}, \texttt{/vision/show\_depth\_image}, \texttt{/vision/sam\_detections} \\
clip\_classifier & Classifies detected regions using CLIP & \texttt{/vision/classify\_all}, \texttt{/vision/classify\_bb}, subscribes to \texttt{/vision/sam\_detections} \\
graspnet\_detector & Estimates grasp poses for detected objects & \texttt{/vision/detect\_grasp}, \texttt{/vision/detect\_grasp\_bb}, subscribes to \texttt{/vision/sam\_detections} \\
scene\_understanding & Analyzes scene relationships and context & \texttt{/vision/understand\_scene}, subscribes to \texttt{/vision/sam\_detections} \\
\hline
\end{tabular} \\

\textbf{Node Details} \\

\textbf{1. SAM Detector Node (\texttt{simple\_sam\_detector})}
Detects objects using the Segment Anything Model (SAM).

\paragraph*{Services}
\begin{itemize}
    \item \texttt{/vision/run\_pipeline}: Trigger full SAM pipeline and continuously publish to \texttt{/vision/sam\_detections}  
    Example: \texttt{ros2 service call /vision/run\_pipeline std\_srvs/srv/Trigger}
    
    \item \texttt{/vision/detect\_objects}: Detect objects in a single frame and return bounding boxes, confidences, and distances  
    Example: \texttt{ros2 service call /vision/detect\_objects custom\_interfaces/srv/DetectObjects}
    
    \item \texttt{/vision/show\_depth\_image}: Display depth camera visualization for debugging  
    Example: \texttt{ros2 service call /vision/show\_depth\_image std\_srvs/srv/Trigger}
\end{itemize}
...
\subsubsection*{2. CLIP Vision Classifier Node (\texttt{clip\_classifier})}
Classifies image regions using the CLIP model.

\paragraph*{Services}
\begin{itemize}
    \item \texttt{/vision/classify\_all}: Classify the entire camera frame  
    Example: \texttt{ros2 service call /vision/classify\_all std\_srvs/srv/Trigger}
    
    \item \texttt{/vision/classify\_bb}: Classify a bounding box region  
    Example: \texttt{ros2 service call /vision/classify\_bb custom\_interfaces/srv/ClassifyBBox "\{x1:100, y1:100, x2:200, y2:300\}"}
    
    \item \texttt{/vision/run\_pipeline}: Subscribes to \texttt{/vision/sam\_detections} and automatically classifies regions
\end{itemize}

\subsubsection*{3. GraspNet Detector Node (\texttt{graspnet\_detector})}
Estimates grasp poses for detected objects.

\paragraph*{Services}
\begin{itemize}
    \item \texttt{/vision/detect\_grasp}: Compute grasp poses for all detected objects
    \item \texttt{/vision/detect\_grasp\_bb}: Compute grasp pose within a specific bounding box
    \item \texttt{/vision/run\_pipeline}: Subscribes to \texttt{/vision/sam\_detections} and runs grasp detection automatically
\end{itemize}


\textbf{4. Scene Understanding Node (\texttt{scene\_understanding})}
Analyzes spatial relationships and scene context.

\textbf{Services and Publications}
\begin{itemize}
    \item \texttt{/vision/understand\_scene}: Provides scene-level understanding
    \item \texttt{/vision/run\_pipeline}: Subscribes to \texttt{/vision/sam\_detections} and executes full scene analysis
    \item Publishes: \texttt{/vision/scene\_understanding} (\texttt{SceneUnderstanding} message)
\end{itemize}
...
Despite the successful integration of the Image Processing pipeline, several limitations remain. 
